...
class HXConnector {
  ...
  init() {
      this.connectAttributes(
          document.querySelectorAll("[hx-trigger][hx-vals]")
      );
      document.body.addEventListener("htmx:afterSwap", (event) => {
          const newElements = event.detail.target.querySelectorAll(
              "[hx-trigger][hx-vals]"
          );
          this.connectAttributes(newElements);
      });
  }
   ...
   *
   * @param {NodeList} elements - A list of elements to connect attributes for.
   */
                      ...
                      values.attributes.forEach(attributeSet => {
                          ...
                          if (element) {
                              Object.keys(attributeSet.attributes).forEach(attr => {
                                  if (attr === "class") {
                                      this.updateClassAttribute(element, attributeSet.attributes[attr]);
                                  } else {
                          ...
                          values.swaps.forEach(swap => {
                              ...
                              if (element) {
                                  Object.keys(swap.attributes).forEach(attr => {
                                      if (attr === "class") {
                                          this.updateClassAttribute(element, swap.attributes[attr]);
                                      } else {
   ...
   *
   * @param {HTMLElement} element - The element to update.
   * @param {string} className - The class name(s) to add or remove.
   */
